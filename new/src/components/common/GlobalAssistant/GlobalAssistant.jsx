    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages(prev => [...prev, {
                id: Date.now() + 1,
                text: language === 'ar' ? 'عذراً، واجهت مشكلة تقنية.' : 'Sorry, I encountered an error.',
                isBot: true
            }]);
        } finally {
            setIsTyping(false);
                ...
                <div className="assistant-window">
                    <div className="window-header">
                        ...
                        <div className="info">
                            ...
                            <p>{isTyping ? (language === 'ar' ? 'جاري التفكير...' : 'Thinking...') : (language === 'ar' ? 'متصل' : 'Online')}</p>
                        </div>
                    </div>

                    <div className="chat-area">
                        {messages.map(msg => (
                            <div key={msg.id} className={`assistant-message ${msg.isBot ? 'bot' : 'user'}`}>
                                {msg.text}
                                {msg.nav && (
