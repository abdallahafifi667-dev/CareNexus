...
const ForgotPassword = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    const resultAction = await dispatch(
      createNewPassword({ email: email.trim(), password }),
    );

    if (createNewPassword.fulfilled.match(resultAction)) {
      toast.success(
        t("auth.reset_success_msg", "Password Reset Successfully!"),
      );
      ...
      <div className="forgot-password-card">
        ...
        {resetPhase === "email" && (
          <form className="forgot-password-form" onSubmit={handleEmailSubmit}>
            ...
            <button
              type="submit"
              className="forgot-password-btn flex justify-center items-center"
              disabled={isLoading}
            >
              {isLoading ? (
                <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
              ) : (
                t("auth.send_code_btn", "Send Recovery Code")
              )}
            </button>
          </form>
        )}
        ...
        {resetPhase === "code" && (
          <form className="forgot-password-form" onSubmit={handleCodeSubmit}>
            <div className="mb-4 p-4 bg-blue-50 border border-blue-100 rounded-xl text-blue-700 text-sm">
              {t("auth.code_sent_to", "A code has been sent to")}:{" "}
              <b>{email}</b>
              <button
                onClick={() => {
                  dispatch(resetPasswordPhase("email"));
            ...
            >
              {isLoading ? (
                ...
                t("auth.verify_code_btn", "Verify Code")
              )}
            </button>
          </form>
        )}
        ...
        {resetPhase === "new_password" && (
          ...
          >
            ...
            <button
              type="submit"
              className="forgot-password-btn flex justify-center items-center mt-2"
              disabled={isLoading}
            >
              {isLoading ? (
                <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
        ...
        {resetPhase === "success" && (
          <div className="text-center py-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-2">
              {t("auth.reset_success_title", "Success!")}
            </h2>
            <p className="text-gray-600 mb-6">
              {t("auth.reset_success_msg", "Your password has been changed.")}
            </p>
            <Link
              to="/auth/login"
              className="forgot-password-btn inline-block text-center"
            >
